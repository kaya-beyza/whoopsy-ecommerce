...
public class CreatePaymentCommandHandler : IRequestHandler<CreatePaymentCommand, PaymentDto>
{
    ...
    public async Task<PaymentDto> Handle(CreatePaymentCommand request, CancellationToken cancellationToken)
    {
        ...
        // 8) Ödeme başarılıysa sipariş onay maili gönder
        //    (CreateOrder'dan buraya taşındı — başarısız ödemede mail gitmemesi için)
        if (paymentResponse.IsSuccess && !string.IsNullOrWhiteSpace(order.User?.Email))
        {
            var html = EmailTemplates.OrderConfirmation(order);
            var subject = $"whoopsy - Siparişin alındı (#{order.Id.ToString()[..8].ToUpper()})";
            await _emailService.SendAsync(order.User.Email, subject, html, cancellationToken);
        }

        // 9) Sonucu DTO olarak döndür
